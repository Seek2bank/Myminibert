...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### DONE
    add_norm = ln_layer(input + dropout(dense_layer(output)))
    return add_norm
    # raise NotImplementedError
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    attn_add_norm = self.add_norm(hidden_states, attn, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    feedforward = self.interm_af(self.interm_dense(attn_add_norm))# self.interm_dense(self.interm_af(attn_add_norm))
    fd_add_norm = self.add_norm(attn_add_norm, feedforward, self.out_dense, self.out_dropout, self.out_layer_norm)
    return fd_add_norm
    # raise NotImplementedError
